...
class CatalogViewModelTest {
    ...
    private fun createViewModel(): CatalogViewModel {
        ...
        val mockQuery = mockk<Query>(relaxed = true) {
            every { get() } returns Tasks.forResult(mockQuerySnapshot)
        }
        val mockDocRef = mockk<DocumentReference>(relaxed = true) {
            every { set(any()) } returns Tasks.forResult(null)
            every { set(any(), any()) } returns Tasks.forResult(null)
            every { update(any<String>(), any()) } returns Tasks.forResult(null)
        }
        val mockCollection = mockk<CollectionReference>(relaxed = true) {
            every { document(any()) } returns mockDocRef
            every { whereEqualTo(any<String>(), any()) } returns mockQuery
        }
        val mockFirestore = mockk<FirebaseFirestore>(relaxed = true) {
            every { collection(any()) } returns mockCollection
        }
        val mockStatusVM = mockk<StatusViewModel>(relaxed = true)
        return CatalogViewModel(mockDitto, mockFirestore, mockStatusVM)
    }
